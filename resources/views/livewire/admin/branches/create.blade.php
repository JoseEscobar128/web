    ...
    <div class="w-full h-full p-6 bg-mostaza">
        <div class="flex h-full items-center space-x-6 pr-16">
            <div class="w-3/5 flex items-center justify-center">
                <img src="{{ asset('images/registeruser.png') }}" alt="Ilustración de sucursales" class="w-full max-w-xl mr-20">
            </div>
            <div>
                <h2 class="text-black text-2xl font-bold mb-4">Nueva Sucursal</h2>
                <form wire:submit.prevent="saveBranch" class="mb-8">
                    <div class="w-[600px] bg-black/50 backdrop-blur-sm p-8 rounded-[20px] shadow-lg">
                        <div class="grid grid-cols-2 gap-x-8 gap-y-6">
                            <div class="col-span-2">
                                <label for="nombre" class="block text-white font-semibold mb-1">Nombre de la Sucursal</label>
                                <input wire:model="nombre" type="text" id="nombre" placeholder="Ej: Sucursal Centro" class="w-full px-4 py-2 rounded-md border-none focus:ring-2 focus:ring-mostaza">
                                @error('nombre') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>
                            <div class="col-span-2">
                                <label for="direccion" class="block text-white font-semibold mb-1">Dirección</label>
                                <input wire:model="direccion" type="text" id="direccion" placeholder="Ej: Av. Principal #123" class="w-full px-4 py-2 rounded-md border-none focus:ring-2 focus:ring-mostaza">
                                @error('direccion') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label for="ciudad" class="block text-white font-semibold mb-1">Ciudad</label>
                                <input wire:model="ciudad" type="text" id="ciudad" placeholder="Ej: Torreón" class="w-full px-4 py-2 rounded-md border-none focus:ring-2 focus:ring-mostaza">
                                @error('ciudad') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label for="telefono" class="block text-white font-semibold mb-1">Teléfono</label>
                                <input wire:model="telefono" type="tel" id="telefono" placeholder="Ej: 8711234567" class="w-full px-4 py-2 rounded-md border-none focus:ring-2 focus:ring-mostaza">
