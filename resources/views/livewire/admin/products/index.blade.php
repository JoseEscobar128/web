<div x-data="{ showConfirmModal: @entangle('showConfirmModal') }">
    <div class="px-8 sm:px-12 lg:px-16 pb-12">
        ...
        <!-- CABECERA CON TÍTULO, FILTROS Y BOTÓN -->
        <div class="bg-white p-6 rounded-lg shadow-sm mt-8 mb-6">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <!-- Título -->
                <div class="flex-shrink-0">
                    <h1 class="text-3xl font-semibold text-black">Productos</h1>
                </div>
                ...
                <div class="flex-grow flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <div class="w-full md:w-auto md:min-w-[200px]">
                        <select wire:model.live="selectedCategory" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                ...
                <div class="flex-shrink-0">
                    <a href="{{ route('admin.products.create') }}" wire:navigate class="flex items-center bg-[#922F06] text-white font-bold py-2 px-4 rounded-lg hover:bg-[#7A2805] transition-colors duration-300">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($products as $product)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm font-semibold text-gray-900">${{ number_format($product['precio'], 2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('admin.products.edit', ['id' => $product['id']]) }}" wire:navigate>
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-center p-6 text-gray-500">No se encontraron productos que coincidan con los filtros.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
