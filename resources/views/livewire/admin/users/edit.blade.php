<div>
    <div class="w-full h-full p-6 bg-mostaza">
        <div class="flex h-full items-center space-x-6 pr-16">
            ...
            <div>
                <h2 class="text-black text-2xl font-bold mb-4">Editar usuario</h2>
                <form wire:submit.prevent="updateUser" class="mb-8">
                    <div class="w-[600px] bg-black/50 backdrop-blur-sm p-8 rounded-[20px] shadow-lg">
                        <div class="grid grid-cols-2 gap-x-8 gap-y-6">
                            ...
                            <div class="col-span-2">
                                ...
                                <input wire:model.live="usuario" type="text" id="usuario" class="w-full px-4 py-2 rounded-md border-none focus:ring-2 focus:ring-mostaza @error('usuario') ring-2 ring-red-500 @enderror">
                                @error('usuario') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label for="email" class="block text-white font-semibold mb-1">Email</label>
                                <input wire:model.live="email" type="email" id="email" class="w-full px-4 py-2 rounded-md border-none focus:ring-2 focus:ring-mostaza @error('email') ring-2 ring-red-500 @enderror">
                                @error('email') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label for="role" class="block text-white font-semibold mb-1">Rol</label>
                                {{-- LÓGICA CONDICIONAL PARA EL ROL --}}
                                @if(in_array($role_name, ['SUPERADMIN', 'ADMIN_SUC']))
                                    <input type="text" value="{{ $role_name }}" class="w-full px-4 py-2 rounded-md border-none bg-gray-600 text-gray-300 cursor-not-allowed" disabled>
                                    <p class="text-xs text-gray-400 mt-1">Este rol no se puede modificar.</p>
                                @else
                            ...
                            <div>
                                <label for="password" class="block text-white font-semibold mb-1">Nueva Contraseña (Opcional)</label>
                                <input wire:model.live="password" type="password" placeholder="Dejar en blanco para no cambiar" class="w-full px-4 py-2 rounded-md border-none focus:ring-2 focus:ring-mostaza @error('password') ring-2 ring-red-500 @enderror">
                                @error('password') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label for="password_confirmation" class="block text-white font-semibold mb-1">Confirmar Nueva Contraseña</label>
                                <input wire:model.live="password_confirmation" type="password" class="w-full px-4 py-2 rounded-md border-none focus:ring-2 focus:ring-mostaza @error('password') ring-2 ring-red-500 @enderror">
                            </div>
